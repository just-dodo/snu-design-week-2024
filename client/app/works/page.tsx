  ...
  return (
    <div className="bg-white w-full flex flex-col md:grid md:grid-cols-2 md:grid-rows-6 border-b border-r">
      <div className="border-l border-primary">
        <p className="mt-8 ml-8 text-4xl text-primary">VD</p>
      </div>
      <div className="border-l border-primary">
        <p className="pt-8 pl-8 text-4xl text-primary">ID</p>
      </div>
    ...
    <div className="flex flex-col h-full w-full">
      <div className="flex flex-col">
        <div className="bg-secondary w-full h-[63px] flex items-center">
          <p className="text-2xl font-medium text-primary ml-6">VD</p>
        </div>
        {courseList.map((course, index) => {
          // course.english_text to kebab case
          const coursePath = course.path;
      ...
      <div className="flex flex-col">
        <div className="bg-primary w-full h-[63px] flex items-center">
          <p className="text-2xl font-medium text-white ml-6">ID</p>
        </div>
        {courseList.map((course, index) => {
          // course.english_text to kebab case
          const coursePath = course.path;
...
const WorksPage: NextPage = () => {
  ...
  return (
    <div className="bg-white w-full h-full flex flex-row">
      ...
      {windowSize.width > 768 ? <DesktopClassList /> : <MobileClassList />}
...
function Course({
  ...
  return (
    <Link
      className="relative pt-4 pl-8 border-t border-l border-primary bg-transparent z-10 overflow-hidden w-full h-full "
      key={index}
      href={`/works/${coursePath}`}
      onPointerEnter={() => {
        playWithDirection(1);
