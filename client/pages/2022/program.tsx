                ...
                <XWrapper>
                  <div className='flex flex-col px-4 py-8 md:p-0 m-0 md:mt-12  no-scrollbar'>
                    {barTextPair.paragraphs.map((paragraph, blockIndex) => {
                      return (
                        <div className={'flex flex-col ' + ((index==2  && !isMobile )? ( blockIndex == 0 ? " mb-[40px]" : " h-[180px]") : "" )} key={"wrapper" + blockIndex}>
                          {paragraph.header ? (
                            <div className='flex flex-row mb-6'>
                              <p className='text-base font-bold'>{paragraph.header}</p>
                            </div>
                          ) : null}
                          <div>
                          </div>
                          ...
                          >
                            <div className="flex flex-1 flex-row h-full justify-start items-start mb-6">
                              <p className={`whitespace-pre-wrap break-keep md:pr-16 mr-auto ${paragraph.left.type == "bold" ? "text-base font-bold" : "mytext-1 text-sm"}`}>{paragraph.left.text}</p>
                            </div>
                            <div className="flex flex-1 flex-row w-full h-full justify-start items-start mb-12">
                                <p className='whitespace-pre-wrap break-keep md:pr-16 mytext-1 text-myblack'>{paragraph.right.text}</p>
                            </div>
                          </div>
                        </div>
                      )
